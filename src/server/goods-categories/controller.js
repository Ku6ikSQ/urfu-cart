...
export default class GoodCategoryController {
    ...
    static async createCategory(req, res) {
        try {
            ...
            const category = await GoodCategoryService.createCategory(title, description, parentId);
    ...
    static async updateCategory(req, res) {
        try {
            const { id } = req.params;
            const { title, description, parentId } = req.body;
            const category = await GoodCategoryService.updateCategory(id, title, description, parentId);
            return res.json(category);
        } catch (e) {
            return res.status(400).json(e.message);
        }
