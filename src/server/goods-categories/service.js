...
export default class GoodCategoryService {
    static async getCategoryById(id) {
        const result = await db.query("SELECT * FROM goods_categories WHERE id = $1", [id]);
    ...
    static async deleteCategory(id) {
        const result = await db.query("DELETE FROM goods_categories WHERE id = $1 RETURNING *", [id]);
        if (!result.rows[0]) throw new Error("Category not found");
    }
}
