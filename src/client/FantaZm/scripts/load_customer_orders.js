document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function getCustomerOrders(userId) {
        try{
            ...
            const data = await response.json();
            const orders = data.filter(item => item.user_id == userId);
            return orders;
        } catch (error) {
            console.error(error);
            return null;
    ...
    async function retryOrder(cartId, recipientId, deliveryId, paymentId) {
        try{
            const cart = await getUserCart(userId);
            if (cart.items.length > 0) {
                for (const item of cart.items) {
                    const response = await fetch(`https://5.35.124.24:5000/api/cart/${cart.cart_id}/${item.goods_id}`, {
                        method: 'DELETE'
                    });
                    if (!response.ok) {
                        throw new Error(`Ошибка HTTP: ${response.status}`);
            ...
            const realCart = await getCart(cartId);
            for (const item of realCart.items) {
                const realGood = await getGood(item.goods_id);
                const realCount = realGood.stock < item.count ? realGood.stock : item.count;
                if (realCount==0){
                    continue;
                }
                const postResponse = await fetch(`https://5.35.124.24:5000/api/cart/${cart.cart_id}/items`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 'goodsId': item.goods_id, 'count': realCount, 'selected': true })
                });
                if (!postResponse.ok) {
                    throw new Error(`Ошибка HTTP: ${postResponse.status}`);
                }
                const postData = await postResponse.json();
                const patchMetricResponse = await fetch(`https://5.35.124.24:5000/api/metrics/${item.goods_id}`,{
                    method:'PATCH',
                    headers : {
                        'Content-Type':'application/json'
                    },
                ...
                if (!patchMetricResponse.ok) {
                    console.log(patchMetricResponse);
                }
                const ret = await patchMetricResponse.json();
            }
            const orderData = {
    ...
    async function createOrderCard(order) {
        ...
        goodsPrice.appendChild(button);
        orderGoods.appendChild(goodsPrice);
        card.appendChild(orderGoods);

        open.addEventListener('click', () => openOrder(card, open)); 
        button.addEventListener('click', () => retryOrder(order.cart_id, order.recipient_id, order.delivery_id, order.payment_id));
        return card;
    }
    ...
    async function loadOrders() {
        ...
        orders.innerHTML = '';
        const ordersData = await getCustomerOrders(userId);
